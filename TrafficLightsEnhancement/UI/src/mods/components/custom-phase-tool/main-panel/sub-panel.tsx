...
import { callUpdateCustomPhaseData, callHighlightEdge, callSetMainPanelState } from "bindings";
...
import { MainPanelItemTitle, MainPanelItemCustomPhase, EdgeInfo } from "mods/general";
...
export const ItemTitle = (props: {title: string, secondaryText?: string, tooltip?: React.ReactNode, dim?: boolean}) => {
...
export function TrafficLightModeSelector(props: {trafficLightMode: number}) {
  ...
  return (
    <PanelFoldout header={<div className={styles.foldoutHeader}>{getString(locale, "TrafficLightMode")}</div>} initialExpanded={true}>
      <MainPanelRadio
  ...
  return (
    <>
      {!props.statisticsOnly && <>
        ...
        <Divider />
        <PanelFoldout header={<div className={styles.foldoutHeader}>{getString(locale, "PhaseChangeMode")}</div>} initialExpanded={false}>
            <MainPanelRadio
        ...
        <Divider />
        <PanelFoldout header={<div className={styles.foldoutHeader}>Adjustments</div>} initialExpanded={false}>
          <MainPanelRange className={styles.hover} data={{
            itemType: "range",
            key: "MinimumDuration",
        ...
        {data.trafficLightMode === 0 && <>
          <Divider/>
          <PanelFoldout header={<div className={styles.foldoutHeader}>{getString(locale, "VehicleWeights")}</div>} initialExpanded={false}>
            <MainPanelRange
            className={styles.hover}
              keyName="CarWeight"
              label="Car Weight"
        ...
        <PanelFoldout header={<div className={styles.foldoutHeader}>Statistics</div>} initialExpanded={true}>
          <ItemTitle title="Timer" secondaryText={`${data.timer}s / ${Math.round(Math.min(Math.max(data.targetDuration, data.minimumDuration), data.maximumDuration))}s`} dim={true} />
          ...
          <Divider />
          <ItemTitle title="Flow" secondaryText={`${Round(data.carFlow)}`} dim={true} tooltip="Average car flow through this phase" />
          <ItemTitle title="Flow Ratio" secondaryText={`${Round(data.flowRatio)}`} dim={true} tooltip="Smoothed flow ratio for phase decisions" />
          <ItemTitle title="Wait Ratio" secondaryText={`${Round(data.waitRatio)}`} dim={true} tooltip="Smoothed wait ratio for phase decisions" />
          <ItemTitle title="Weighted Waiting" secondaryText={`${Round(data.weightedWaiting)}`} dim={true} tooltip="Combined waiting metric used for phase priority" />
          <Divider />
        ...
        {props.edges && props.edges.length > 0 && props.phaseIndex !== undefined && <>
          <Divider />
          <PanelFoldout header={<div className={styles.foldoutHeader}>Signal Delays</div>} initialExpanded={false}>
